...
class NSEPRI:
    ...
    def missing_close_indices(
        self,
        csv_file: str
    ) -> dict[str, list[str]]:
        '''
        Identify indices missing between the downloaded summary file and the
        in-built base file, in both directions.
        ...
        -------
        dict
            Dictionary containing two keys with lists of indices missing from
            the downloaded file and the base file, respectively.
        '''
        ...
        # Missing indices
        col = 'Index Name'
        missing_close = df[~df[col].isin(base_df[col].tolist())][col].tolist()
        missing_base = base_df[~base_df[col].isin(df[col].tolist())][col].tolist()

        output = {
            'Missing index from summary report': missing_close,
            'Missing index from base file': missing_base
        }
